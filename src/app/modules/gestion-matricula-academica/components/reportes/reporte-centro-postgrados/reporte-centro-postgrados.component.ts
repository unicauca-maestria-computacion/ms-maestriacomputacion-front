...
export class ReporteCentroPostgradosComponent implements OnInit {
        ...
        this.http.get<ReporteRow[]>(`${this.financieraApi}/reporte-centro-postgrados/${periodoId}`).subscribe({
            next: async (data) => {
                try {
                    ...
                    this.messageService.add({ severity: 'success', summary: 'Reporte generado', detail: 'La descarga comenzará en breve' });
                } catch (e) {
            ...
            error: () => {
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'No se pudieron obtener los datos del reporte' });
                this.descargando = false;
            },
        });
    }
    ...
    private async generarExcel(data: ReporteRow[]): Promise<void> {
        ...
        ws.columns = [
            { width: 3 },  { width: 3 },  { width: 14 }, { width: 35 },
            { width: 10 }, { width: 10 }, { width: 14 }, { width: 14 },
            { width: 18 }, { width: 10 }, { width: 3 },  { width: 10 },
            { width: 38 }, { width: 3 },  { width: 38 }, { width: 10 },
        ];
        ...
        const headers = ['', '', 'Identificación', 'Nombres completos del estudiante',
            'Valor de Matrícula en SMMLV', 'SEM FINAN', '¿Aplica Descuento de Voto?',
            'Descuento por Egresado (UNICAUCA)', 'No. Res. de Beca', '% Beca',
            '', 'SEM ACAD', 'Materias a Matricular', '',
            'Nombre Completo de Docente encargado', 'Grupo Clase'];
        ...
        r6.getCell(13).font = { italic: true, size: 8, color: { argb: 'FF666666' } };
        ...
        for (const row of data) {
            ...
            const dataRow = ws.addRow([
                '', '', row.identificacion, row.nombreCompleto,
                row.valorMatriculaSMMLV, row.semestreFinanciero,
        ...
        // Row height for data
        ws.eachRow((r, i) => { if (i > 6) r.height = 22; });
